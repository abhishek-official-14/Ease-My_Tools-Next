  ...
  return (
    <div
      className={`rounded-2xl border p-3 shadow-xs transition-all ${
        active
          ? "border-blue-500/60 bg-blue-50/40 dark:border-blue-400/60 dark:bg-blue-900/20"
    ...
    >
      ...
      <div className={`mt-1 text-sm font-bold tabular-nums ${colors[accent]}`}>
        {value}
      </div>
    </div>
  )
...
const Panel: React.FC<{
  ...
  <div
    className={`flex flex-col rounded-2xl border border-slate-200/80 bg-white/50 shadow-xs dark:border-slate-800 dark:bg-slate-900/30 ${className}`}
  >
    <div className="flex flex-shrink-0 items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-800/80">
      ...
      {onClear && (
        <button
          onClick={onClear}
          className="rounded-lg p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-500 dark:hover:bg-rose-950/20 dark:hover:text-rose-400 transition"
          title="Clear section"
        >
...
/* ---------- Main component ---------- */
export default function MathToolkit({ tool }: ToolHeroProps) {
  /* Universal converter */
    ...
    <div className="flex justify-center bg-gradient-to-br from-slate-50 via-white to-slate-100 px-4 py-8 text-slate-900 sm:px-6 lg:py-12 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 dark:text-slate-100">
      <div className="w-full max-w-5xl space-y-6">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/90 shadow-xl shadow-slate-200/40 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/90 dark:shadow-black/40">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-bold text-slate-900 dark:text-slate-100">
                  Math Conversion Studio
                </h1>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Decimals, fractions, percentages, and mixed numbers
                </p>
              </div>
            </div>
          ...
          {/* Mobile tabs */}
          <div className="flex border-b border-slate-100 lg:hidden dark:border-slate-800">
            <button
          ...
          <div className="p-6">
            <div className="grid gap-6 lg:grid-cols-[1fr_0.85fr]">
              ...
              <div
                className={
                  activeTab === "universal" ? "" : "hidden lg:block"
                }
              >
                <Panel
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {(
                          ["decimal", "fraction", "percent", "mixed"] as const
                        ).map((fmt) => (
                          <button
                            key={fmt}
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base font-bold text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                    />
              ...
              {/* Right panel: Fraction Tools */}
              <div
                className={
                  activeTab === "fractionTools" ? "" : "hidden lg:block"
                }
              >
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {fracTool === "simplify" ? (
                      <div className="w-full max-w-xs space-y-4">
                        <label className="block text-center text-xs font-semibold text-slate-600 dark:text-slate-400">
                          Simplify Fraction
                        </label>
                        <div className="flex items-center justify-center gap-2">
                          <input
                            ...
                            placeholder="Num"
                            className="w-20 rounded-xl border border-slate-200 bg-white px-3 py-2 text-center text-sm font-bold text-slate-900 outline-none transition focus:border-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                            onKeyDown={(e) =>
                              e.key === "Enter" && handleSimplify()
                            }
                          />
                          <span className="text-sm font-bold text-slate-400">
                            /
                        ...
                        <div className="flex justify-center">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-xs font-semibold text-white shadow-md shadow-blue-500/10 hover:bg-blue-700 active:scale-95 transition"
                          >
                            Simplify
                          </button>
                        </div>
                        {simpResultNum !== null && simpResultDen !== null && (
                          <div className="mt-3 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3 text-center dark:border-slate-800 dark:bg-slate-900/60">
                            ...
                            <div className="mt-1 text-xs text-slate-400">
                              Decimal:{" "}
                              {(simpResultNum / simpResultDen).toFixed(4)}
                            </div>
                          </div>
                        )}
                      </div>
                        ...
                        {mixedDirection === "toImproper" ? (
                          <div className="flex items-center justify-center gap-2">
                            <input
                              ...
                              placeholder="Whole"
                              className="w-16 rounded-xl border border-slate-200 bg-white px-2 py-2 text-center text-sm font-bold text-slate-900 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                            />
                            <input
                              type="number"
                              value={mNum}
                            ...
                            <input
                              ...
                              className="w-16 rounded-xl border border-slate-200 bg-white px-2 py-2 text-center text-sm font-bold text-slate-900 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                              onKeyDown={(e) =>
                                e.key === "Enter" && handleMixedConvert()
                              }
                            />
                          </div>
                        ) : (
                          <div className="flex items-center justify-center gap-2">
                            <input
                              ...
                              placeholder="Num"
                              className="w-20 rounded-xl border border-slate-200 bg-white px-2 py-2 text-center text-sm font-bold text-slate-900 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                            />
                            <span className="text-sm font-bold text-slate-400">
                              /
                            </span>
