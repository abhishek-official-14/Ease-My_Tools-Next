...
function generateWords(count: number, startLorem: boolean): string {
    ...
    if (startLorem && count >= 2) {
        ...
        for (let i = 2; i < count; i++) words.push(WORDS[randInt(2, WORDS.length - 1)]);
    } else {
        for (let i = 0; i < count; i++) words.push(WORDS[randInt(0, WORDS.length - 1)]);
    }
    return words.join(" ");
}
...
function applyHtml(text: string, type: "paragraphs" | "words" | "sentences", includeUl: boolean): string {
    if (type === "paragraphs") {
        let html = text
            .split("\n\n")
            .map((p) => `<p>${p}</p>`)
        ...
        if (includeUl) {
            ...
            const items = Array.from({ length: listItems }, () => `<li>${generateWords(randInt(2, 5), false)}</li>`).join("\n");
            html += `\n<ul>\n${items}\n</ul>`;
        }
        return html;
    }
...
const StatCard = ({ label, value, accent }: { label: string; value: string; accent: "emerald" | "blue" | "violet" | "slate" }) => {
    const accentMap = {
    ...
    return (
        <div className="flex flex-col gap-0.5 rounded-lg border border-slate-200/80 bg-white/50 px-3 py-2 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/50">
            ...
            <span className={`text-sm font-bold tabular-nums leading-tight ${accentMap[accent]}`}>{value}</span>
        </div>
    );
};
...
export default function LoremIpsumTool() {
    ...
    const generatedText = useMemo(() => {
        ...
        else if (type === "sentences") raw = generateSentences(count, startLorem);
    ...
    const paragraphCount = type === "paragraphs" ? count : (type === "sentences" ? 0 : 0);
                    ...
                    <div className="grid gap-5 px-4 py-4 sm:px-6 sm:py-5 lg:grid-cols-[1fr_1.05fr]">
                        ...
                        <div>
                            <div className="h-full rounded-xl border border-slate-200/80 bg-white/50 p-4 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                                {/* Type selector */}
                                <div className="mb-5">
                                    <label className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                                        Generate by
                                    </label>
                                    <div className="grid grid-cols-3 gap-1.5">
                                        {(["paragraphs", "sentences", "words"] as GeneratorType[]).map((t) => (
                                            <button
                                                key={t}
                                                onClick={() => setType(t)}
                                                className={`rounded-lg border px-2 py-1.5 text-xs font-semibold capitalize transition ${type === t
                                ...
                                <div className="mb-5">
                                    <div className="mb-3 flex items-center justify-between">
                                        ...
                                        <span className="text-base font-bold tabular-nums text-slate-800 dark:text-white">{count}</span>
                                    </div>
                                    <input
                                        type="range"
                                        min={1}
                                        max={type === "words" ? 500 : (type === "sentences" ? 200 : 50)}
                                        value={count}
                                    ...
                                    <div className="mt-1 flex justify-between text-[10px] font-medium text-slate-400 dark:text-slate-500">
                                        <span>1</span>
                                        <span>{type === "words" ? 250 : (type === "sentences" ? 100 : 25)}</span>
                                        <span>{type === "words" ? 500 : (type === "sentences" ? 200 : 50)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="mb-5 space-y-3">
                                    <label className="flex items-center justify-between gap-4">
                                        ...
                                        <button
                                            ...
                                            className={`relative inline-flex h-5 w-9 flex-shrink-0 rounded-full border transition-colors duration-200 ${startLorem ? "border-blue-500 bg-blue-500" : "border-slate-300 bg-slate-200 dark:border-slate-700 dark:bg-slate-700"
                                                }`}
                                        >
                                            <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${startLorem ? "translate-x-4" : "translate-x-0"
                                                }`} />
                                        </button>
                                    </label>
                                    ...
                                    <label className="flex items-center justify-between gap-4">
                                        ...
                                        <button
                                            ...
                                            className={`relative inline-flex h-5 w-9 flex-shrink-0 rounded-full border transition-colors duration-200 ${includeHtml ? "border-blue-500 bg-blue-500" : "border-slate-300 bg-slate-200 dark:border-slate-700 dark:bg-slate-700"
                                                }`}
                                        >
                                            <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${includeHtml ? "translate-x-4" : "translate-x-0"
                                                }`} />
                                        </button>
                                    </label>
                                    ...
                                    {includeHtml && type === "paragraphs" && (
                                        <label className="flex items-center justify-between gap-4 pl-2">
                                            ...
                                            <button
                                                ...
                                                onClick={() => setIncludeUl(!includeUl)}
                                                className={`relative inline-flex h-5 w-9 flex-shrink-0 rounded-full border transition-colors duration-200 ${includeUl ? "border-blue-500 bg-blue-500" : "border-slate-300 bg-slate-200 dark:border-slate-700 dark:bg-slate-700"
                                                    }`}
                                            >
                                                <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${includeUl ? "translate-x-4" : "translate-x-0"
                                                    }`} />
                                            </button>
                                        </label>
                                    )}
                        ...
                        <div className="h-full">
                            <div className="flex h-full flex-col rounded-xl border border-slate-200/80 bg-white/50 p-4 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                                {/* Output textarea */}
                                <div className="flex-1 min-h-0 rounded-lg border border-slate-200 bg-white/60 dark:border-slate-800 dark:bg-slate-900/60">
                                    <textarea
                                        readOnly
                                        value={generatedText}
                                ...
                                <div className="mt-4 grid grid-cols-3 gap-2">
                                    <StatCard label="Words" value={wordCount.toString()} accent="blue" />
                                    <StatCard label="Characters" value={charCount.toString()} accent="violet" />
                                    <StatCard label="Paragraphs" value={paragraphCount > 0 ? paragraphCount.toString() : "—"} accent="emerald" />
                                </div>
