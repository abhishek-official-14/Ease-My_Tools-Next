...
const PasswordGenerator = () => {
    ...
    const charPool = useMemo(() => {
        ...
        if (excludeSimilar) {
            pool = pool.split("").filter((ch) => !similarChars.includes(ch)).join("");
        }
        if (excludeAmbiguous) {
            pool = pool.split("").filter((ch) => !ambiguousChars.includes(ch)).join("");
        }
        return pool;
    ...
    const validateSettings = () => {
        ...
        const totalMin = (includeNumbers ? minNumbers : 0) + (includeSymbols ? minSymbols : 0) + (includeUppercase ? minUppercase : 0);
        if (totalMin > length) {
            alert(`Password length (${length}) is too short for the minimum requirements (${totalMin}). Increase length or reduce minimum counts.`);
            return false;
        }
        if (noDuplicate && charPoolSize < length) {
            alert(`Cannot generate a password with no duplicate characters because the character pool (${charPoolSize}) is smaller than the password length (${length}). Include more character types or reduce length.`);
            return false;
        }
        return true;
    };
    ...
    const generatePassword = () => {
        ...
        if (noDuplicate) {
            ...
            if (uniqueSet.size < length) {
                const available = charPool.split("").filter(ch => !uniqueSet.has(ch));
                ...
                if (uniqueSet.size < length) {
                    ...
                    while (arr.length < length) {
                        arr.push(charPool[Math.floor(Math.random() * charPool.length)]);
                    }
    ...
    const calculateStrength = () => {
        ...
        if (score <= 3) return { strength: "Very Weak", level: 1, color: "bg-red-500" };
        if (score <= 5) return { strength: "Weak", level: 2, color: "bg-orange-500" };
        if (score <= 7) return { strength: "Fair", level: 3, color: "bg-yellow-500" };
        if (score <= 9) return { strength: "Good", level: 4, color: "bg-blue-500" };
  ...
  const strength = calculateStrength();
    ...
    useEffect(() => {
        if (charPoolSize > 0) {
            const totalMin = (includeNumbers ? minNumbers : 0) + (includeSymbols ? minSymbols : 0) + (includeUppercase ? minUppercase : 0);
            if (totalMin <= length && (!noDuplicate || charPoolSize >= length)) {
                generatePassword();
            }
        }
    }, [
                    ...
                    <div className="p-5 sm:p-6 space-y-5">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                disabled={!password}
                                className={`rounded-lg px-4 py-2.5 text-sm font-semibold transition active:scale-[0.98] ${copied
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map((level) => (
                                    <div
                                        key={level}
                                        className={`h-1.5 flex-1 rounded-full transition-all ${level <= strength.level ? strength.color : "bg-slate-200 dark:bg-slate-800"
                                            }`}
                                    />
                                ))}
                            </div>
                        ...
                        <div>
                            <div className="mb-2 flex items-center justify-between">
                                ...
                                <span className="text-xs font-bold text-slate-700 dark:text-slate-200">{length}</span>
                            </div>
                            <input
                                type="range"
                                min="4"
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                                <div>
                                    <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">Minimum Numbers</label>
                                    <select
                                        value={minNumbers}
                                        onChange={(e) => setMinNumbers(parseInt(e.target.value))}
                                        disabled={!includeNumbers}
                                        className="w-full rounded-lg border border-slate-200/80 bg-white/60 px-2 py-1.5 text-xs text-slate-800 focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400 dark:border-slate-800/60 dark:bg-slate-900/60 dark:text-slate-100 disabled:opacity-50"
                                    >
                                        {[1, 2, 3, 4].map((n) => (
                                ...
                                <div>
                                    <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">Minimum Symbols</label>
                                    <select
                                        value={minSymbols}
                                ...
                                <div>
                                    <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">Minimum Uppercase</label>
                                    <select
                                        value={minUppercase}
                        ...
                        <div className="rounded-lg border border-slate-200/80 bg-white/50 p-3 dark:border-slate-800/60 dark:bg-slate-900/50">
                            ...
                            <div className="flex flex-wrap justify-between gap-2 text-xs">
                                <div>
                                    <span className="text-slate-500 dark:text-slate-400">Combinations:</span>
                                    <span className="ml-1 font-mono font-bold text-slate-800 dark:text-slate-200">
                                        {formatCombinations(stats.totalCombinations)}
                                    </span>
                                </div>
                                <div>
                                    <span className="text-slate-500 dark:text-slate-400">Character Pool:</span>
                                    <span className="ml-1 font-mono font-bold text-slate-800 dark:text-slate-200">
                                        {charPoolSize} chars
                                    </span>
                                </div>
                                <div>
                                    <span className="text-slate-500 dark:text-slate-400">Entropy:</span>
                                    <span className="ml-1 font-mono font-bold text-slate-800 dark:text-slate-200">
