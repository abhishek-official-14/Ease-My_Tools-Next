...
export default function QRCodeTool() {
    ...
    const [decodePreviewUrl, setDecodePreviewUrl] = useState<string | null>(null);
    ...
    const processDecodeFile = useCallback(
        (file: File) => {
            ...
            if (!allowed.includes(file.type)) {
                setDecodeError("Unsupported format. Please use PNG, JPG, WebP, BMP, or GIF.");
                return;
            }
    ...
    const handleDrop = (e: React.DragEvent) => {
        e.preventDefault();
        setIsDragOver(false);
        if (e.dataTransfer.files?.[0]) processDecodeFile(e.dataTransfer.files[0]);
    };
            ...
            <div className="w-full max-w-5xl">
                <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white/80 shadow-xl shadow-slate-200/40 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/80 dark:shadow-black/30">
                    ...
                    <div className="flex justify-center p-2 lg:hidden">
                        <div className="inline-flex rounded-lg bg-slate-100 p-0.5 dark:bg-slate-800">
                            <button
                                ...
                                className={`rounded-md px-4 py-2 text-xs font-semibold transition ${activeTab === "generate"
                            ...
                            <button
                                ...
                                className={`rounded-md px-4 py-2 text-xs font-semibold transition ${activeTab === "decode"
                                        ? "bg-white text-slate-900 shadow-sm dark:bg-slate-700 dark:text-white"
                                        : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"
                                    }`}
                            >
                    ...
                    <div className="grid gap-4 px-3 py-3 sm:gap-5 sm:px-4 sm:py-4 lg:grid-cols-[1fr_1.05fr]">
                        ...
                        <div className={`${activeTab === "decode" ? "hidden lg:block" : ""}`}>
                            <div className="rounded-xl border border-slate-200/80 bg-white/50 p-3 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                                ...
                                <div className="relative">
                                    <textarea
                                        ...
                                        rows={2}
                                        className="w-full resize-none rounded-lg border border-slate-200 bg-white/80 px-3 py-1.5 pr-9 text-sm font-mono text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-100"
                                    />
                                    {inputText && (
                                        <button
                                            onClick={clearInputText}
                                            className="absolute right-1.5 top-1.5 rounded-md p-1 text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/30"
                                            title="Clear text"
                                        >
                                        ...
                                        <div className="grid grid-cols-4 gap-1">
                                            {(["small", "medium", "large", "xlarge"] as QRSize[]).map((size) => (
                                                <button
                                                    ...
                                                    className={`rounded-md border px-1 py-1 text-xs font-semibold transition ${qrSize === size
                                        ...
                                        <div className="grid grid-cols-4 gap-1">
                                            {(["L", "M", "Q", "H"] as ErrorCorrectionLevel[]).map((level) => (
                                                <button
                                                    ...
                                                    className={`rounded-md border px-1 py-1 text-xs font-semibold transition ${errorLevel === level
                                                            ? "border-blue-500 bg-blue-500 text-white shadow-sm"
                                                            : "border-slate-200 bg-white/60 text-slate-700 hover:border-slate-300 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-800"
                                                        }`}
                                                >
                                ...
                                {hasText ? (
                                    ...
                                    <div className="mt-4 flex flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white/40 p-3 text-center backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/30">
                                        <svg className="mb-1.5 h-6 w-6 text-slate-400 dark:text-slate-500" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                                        ...
                                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">QR will appear here</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className={`${activeTab === "generate" ? "hidden lg:block" : ""}`}>
                            <div className="flex h-full flex-col rounded-xl border border-slate-200/80 bg-white/50 p-3 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                                ...
                                <div
                                    ...
                                    onDrop={handleDrop}
                                    onClick={!hasDecodeFile ? () => decodeFileInputRef.current?.click() : undefined}
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={(e) => {
                                        if (!hasDecodeFile && (e.key === "Enter" || e.key === " ")) decodeFileInputRef.current?.click();
                                    }}
                                    className={`group relative rounded-lg border-2 border-dashed transition-all duration-200 outline-none bg-white/50 dark:bg-slate-900/50
                    ${isDragOver
                  ...
                  `}
                                >
                                    <input
                                        ...
                                        className="hidden"
                                        onChange={(e) => {
                                            if (e.target.files?.[0]) processDecodeFile(e.target.files[0]);
                                        }}
                                    />

                                    {!hasDecodeFile ? (
                                        <div className="flex min-h-[90px] flex-col items-center justify-center text-center p-3">
                                            <div className="mb-2 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 p-1.5 shadow-inner dark:from-slate-800/60 dark:to-slate-900/60">
                                                <svg className="h-5 w-5 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M6.827 6.175A2.31 2.31 0 015.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 00-1.134-.175 2.31 2.31 0 01-1.64-1.055l-.822-1.316a2.192 2.192 0 00-1.736-1.039 48.774 48.774 0 00-5.232 0 2.192 2.192 0 00-1.736 1.039l-.821 1.316z" />
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12.75a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0zM18.75 10.5h.008v.008h-.008V10.5z" />
                                                </svg>
                                            </div>
                                            <p className="text-xs font-semibold text-slate-800 dark:text-white">Drop QR image here</p>
                                            <div className="mt-1 flex flex-wrap justify-center gap-1">
                                                <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-400">PNG</span>
                                                <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-400">JPG</span>
                                                <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-400">WEBP</span>
                                            </div>
                                            <p className="mt-1 text-[9px] font-medium text-slate-400 uppercase dark:text-slate-500">Max 20 MB</p>
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-2 p-2">
                                            <div className="h-10 w-10 flex-shrink-0 overflow-hidden rounded-md border border-slate-200 bg-slate-100 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                                                {decodePreviewUrl && (
                                                    <img src={decodePreviewUrl} alt="preview" className="h-full w-full object-cover" />
                                                )}
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-start justify-between gap-1">
                                                    <div className="min-w-0">
                                                        <p className="truncate text-xs font-semibold text-slate-800 dark:text-white">{decodeFile.name}</p>
                                                        <p className="text-[10px] text-slate-500 dark:text-slate-400">{formatSize(decodeFile.size)}</p>
                                                    </div>
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleDecodeReset(); }}
                                ...
                                {/* Result area */}
                                <div className="mt-3 flex-1 flex flex-col min-h-[60px]">
                                    {hasDecodedText ? (
                                        <>
                                            <div className="flex-1 rounded-lg border border-slate-200/80 bg-white/60 px-2 py-1.5 dark:border-slate-800/60 dark:bg-slate-900/60 overflow-auto">
                                                <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">Decoded</p>
                                                <p className="mt-0.5 break-all text-sm font-mono text-slate-800 dark:text-slate-200">{decodedText}</p>
                                            </div>
                                            <button
                                                onClick={() => {
                                        ...
                                        <div className="flex-1 flex items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white/40 p-2 text-center backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/30">
                                            <p className="text-xs text-slate-400 dark:text-slate-500">Decoded text appears here</p>
                                        </div>
                                    )}
                                </div>

                                {/* Decode button */}
                                {hasDecodeFile && !hasDecodedText && !isDecoding && (
