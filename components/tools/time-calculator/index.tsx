...
const Panel: React.FC<{
    ...
    >
        <div className="flex-shrink-0 border-b border-slate-200 bg-slate-100/80 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50 flex items-center justify-between">
            ...
            {onClear && (
                <button
                    onClick={onClear}
                    className="rounded-md p-1 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:text-rose-400 dark:hover:bg-rose-950/20 transition"
                    title="Clear section"
                >
                    ✕
                </button>
...
const TimePartInput: React.FC<{
    ...
    <div className="flex items-center gap-1">
        <input
            ...
            placeholder="0"
            className={`rounded-lg border border-slate-300 bg-white/80 px-2 py-2 text-sm font-mono text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-100 dark:placeholder:text-slate-500 ${className}`}
        />
        <span className="text-[10px] font-semibold text-slate-500 uppercase dark:text-slate-400 w-5 text-center">
            {label}
        </span>
    </div>
);
...
const DateTimePicker: React.FC<{
    ...
    <div className="flex flex-wrap items-center gap-2">
        <input
            ...
            className="rounded-lg border border-slate-300 bg-white/80 px-2 py-2 text-sm font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-100"
        />
        <input
            type="time"
            value={time}
...
const TimeCalculator: React.FC = () => {
    ...
    const [rows, setRows] = useState([
        { id: 1, sign: "+" as const, days: "", hours: "", minutes: "", seconds: "" },
    ]);
    ...
    const addRow = () => {
        setRows((prev) => [
            ...prev,
            { id: nextRowId, sign: "+" as const, days: "", hours: "", minutes: "", seconds: "" },
        ]);
    ...
    const toggleSign = (id: number) => {
        setRows((prev) =>
            prev.map((r) => (r.id === id ? { ...r, sign: r.sign === "+" ? "-" : "+" } : r))
        );
    };
    ...
    const clearAddSubtract = () => {
        setBaseDate("");
        setBaseTime("12:00");
        setRows([{ id: 1, sign: "+", days: "", hours: "", minutes: "", seconds: "" }]);
        setNextRowId(2);
    };
    ...
    const [convTotalSeconds, setConvTotalSeconds] = useState<number | null>(null);
                ...
                <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white/80 shadow-xl shadow-slate-200/40 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/80 dark:shadow-black/30">
                    ...
                    <div className="flex border-b border-slate-200/80 lg:hidden dark:border-slate-800/60">
                        <button
                            ...
                            className={`flex-1 py-3 text-center text-xs font-medium transition ${activeTab === "calculator"
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 text-center text-xs font-medium transition ${activeTab === "converter"
                                    ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400"
                                    : "text-slate-500 dark:text-slate-400"
                                }`}
                        >
                    ...
                    <div className="p-5 sm:p-6">
                        <div className="grid gap-5 lg:grid-cols-2">
                            {/* Left: Calculator */}
                            <div className={activeTab === "calculator" ? "" : "hidden lg:block"}>
                                <Panel
                                    title="📅 Date & Time Calculator"
                                    onClear={calcMode === "addSubtract" ? clearAddSubtract : clearDifference}
                                >
                                    {/* Mode toggle */}
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => setCalcMode("addSubtract")}
                                            className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all ${calcMode === "addSubtract"
                                        ...
                                        <button
                                            ...
                                            className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all ${calcMode === "difference"
                                                    ? "border-blue-500 bg-blue-500 text-white shadow-sm"
                                                    : "border-slate-200 bg-white/60 text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-200 dark:hover:bg-slate-800"
                                                }`}
                                        >
                                    ...
                                    {calcMode === "addSubtract" && (
                                        <>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <button
                                                    onClick={() => setUseDateMode(!useDateMode)}
                                                    className={`relative inline-flex h-5 w-9 rounded-full border transition-colors ${useDateMode
                                                            ? "border-blue-500 bg-blue-500"
                                                            : "border-slate-300 bg-slate-200 dark:border-slate-700 dark:bg-slate-700"
                                                        }`}
                                                >
                                                    <span
                                                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform ${useDateMode ? "translate-x-4" : "translate-x-0"
                                                            }`}
                                                    />
                                                </button>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="space-y-2">
                                                    {rows.map((row) => (
                                                        <div key={row.id} className="flex items-center gap-2 flex-wrap">
                                                            <button
                                                                ...
                                                                className={`w-8 rounded-lg border px-2 py-1.5 text-xs font-bold transition ${row.sign === "+"
                                                            ...
                                                            <TimePartInput label="d" value={row.days} onChange={(v) => updateRowField(row.id, "days", v)} />
                                                            <TimePartInput label="h" value={row.hours} onChange={(v) => updateRowField(row.id, "hours", v)} max={23} />
                                                            <TimePartInput label="m" value={row.minutes} onChange={(v) => updateRowField(row.id, "minutes", v)} max={59} />
                                                            <TimePartInput label="s" value={row.seconds} onChange={(v) => updateRowField(row.id, "seconds", v)} max={59} />
                                                            {rows.length > 1 && (
                                                                <button onClick={() => removeRow(row.id)} className="p-1 text-slate-400 hover:text-rose-500">
                                                                    ✕
                                                                </button>
                                                            )}
                                                        </div>
                                                    ...
                                                    <button
                                                        onClick={addRow}
                                                        className="w-full rounded-lg border border-dashed border-slate-300 bg-white/60 px-3 py-2 text-xs font-semibold text-slate-600 hover:border-blue-400 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-300 dark:hover:border-blue-500 dark:hover:text-blue-400 transition"
                                                    >
                                                        + Add duration
                                                    </button>
                                                </div>
                                                    ...
                                                    resultingDate ? (
                                                        <>
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <span className="text-lg font-bold tabular-nums text-slate-800 dark:text-white">
                                                                    {formatResultDate(resultingDate)}
                                                                </span>
                                                            </div>
                                                            <div className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                                                                Offset: {formatDuration(totalOffsetSeconds)}
                                                            </div>
                                                        </>
                                                    ) : (
                                                        <span className="text-sm text-slate-400">Pick a start date</span>
                                                    )
                                                ) : (
                                                    <>
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <span className="text-lg font-bold tabular-nums text-slate-800 dark:text-white">
                                                                {formatDuration(totalOffsetSeconds)}
                                                            </span>
                                                        </div>
                                                        <div className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                                                            {formatDurationDetailed(totalOffsetSeconds)}
                                                        </div>
                                                    </>
                                                )}
                                            </div>
                                            ...
                                            {diffResult !== null && (
                                                <div className="rounded-lg border border-slate-200/80 bg-slate-50/50 p-3 dark:border-slate-800/60 dark:bg-slate-900/30">
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <span className="text-lg font-bold tabular-nums text-slate-800 dark:text-white">
                                                            {formatDuration(Math.abs(diffResult))}
                                                        </span>
                                                    </div>
                                                    <div className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                                                        {formatDurationDetailed(Math.abs(diffResult))}
                                                    </div>
                                                    <div className="mt-2 grid grid-cols-2 gap-2">
                                                        <StatCard label="Total Days" value={(diffResult! / 86400).toFixed(2)} accent="slate" />
                                                        <StatCard label="Total Hours" value={(diffResult! / 3600).toFixed(2)} accent="emerald" />
                                                        <StatCard label="Total Minutes" value={(diffResult! / 60).toFixed(2)} accent="blue" />
                                                        <StatCard label="Total Seconds" value={Math.abs(diffResult!).toFixed(0)} accent="violet" />
                                                    </div>
                                                </div>
                                            )}
                                        </>
                            ...
                            {/* Right: Converter */}
                            <div className={activeTab === "converter" ? "" : "hidden lg:block"}>
                                <Panel title="🔄 Time Converter" onClear={clearConverter}>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full rounded-lg border border-slate-300 bg-white/80 px-3 py-2 text-sm font-mono text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-100 dark:placeholder:text-slate-500"
                                        />
                                        <p className="mt-1 text-[10px] text-slate-400 dark:text-slate-500">
                                            Supports: d (days), h (hours), m (minutes), s (seconds), y (years), mo (months), w (weeks), HH:MM:SS
                                        </p>
                                    </div>

                                    {conversions && (
                                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                                            <StatCard label="Milliseconds" value={conversions.ms.toFixed(0)} accent="slate" />
                                            <StatCard label="Seconds" value={conversions.sec.toFixed(2)} accent="slate" />
                                            <StatCard label="Minutes" value={conversions.min.toFixed(2)} accent="emerald" />
                                            <StatCard label="Hours" value={conversions.hr.toFixed(2)} accent="blue" />
                                            <StatCard label="Days" value={conversions.days.toFixed(2)} accent="violet" />
                                            <StatCard label="Weeks" value={conversions.weeks.toFixed(2)} accent="slate" />
                                            <StatCard label="Months (30d)" value={conversions.months.toFixed(2)} accent="emerald" />
                                            <StatCard label="Years (365d)" value={conversions.years.toFixed(2)} accent="blue" />
                                        </div>
                                    )}
                                </Panel>
                            </div>
