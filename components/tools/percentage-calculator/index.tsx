...
const improperToMixed = (num: number, den: number): [number, number, number] => {
...
const mixedToImproper = (whole: number, num: number, den: number): [number, number] => {
    if (den === 0) return [0, 1];
...
const FractionDisplay: React.FC<{
        ...
        return (
            <span className={`inline-flex items-center gap-0.5 ${className}`}>
                ...
                <span className="inline-flex flex-col items-center leading-tight">
                    <span className="border-b border-current px-1 text-[0.85em]">{absNum}</span>
    ...
    return (
        <span className={`inline-flex items-center gap-1 ${className}`}>
            ...
            <span className="inline-flex flex-col items-center leading-tight">
                <span className="border-b border-current px-1 text-[0.85em]">{displayNum}</span>
...
const StatCard: React.FC<{
    ...
    return (
        <div
            className={`rounded-lg border px-3 py-2 shadow-sm transition-all ${active
...
const MathToolkit: React.FC = () => {
    ...
    // Mixed ↔ Improper
    const [mixedDirection, setMixedDirection] = useState<"toImproper" | "toMixed">("toImproper");
    ...
    /* ========== Mobile tabs ========== */
    const [activeTab, setActiveTab] = useState<"universal" | "fractionTools">("universal");
                ...
                <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white/80 shadow-xl shadow-slate-200/40 backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/80 dark:shadow-black/30">
                    ...
                    <div className="flex border-b border-slate-200/80 lg:hidden dark:border-slate-800/60">
                        <button
                            ...
                            className={`flex-1 py-3 text-center text-xs font-medium transition ${activeTab === "universal"
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 text-center text-xs font-medium transition ${activeTab === "fractionTools"
                                    ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-400"
                                    : "text-slate-500 dark:text-slate-400"
                                }`}
                        >
                    ...
                    <div className="p-5 sm:p-6">
                        <div className="grid gap-5 lg:grid-cols-[1fr_0.85fr]">
                            ...
                            <div className={activeTab === "universal" ? "" : "hidden lg:block"}>
                                <Panel title="🔢 Universal Converter" onClear={clearUniversal}>
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1.5">
                                                {(["decimal", "fraction", "percent", "mixed"] as const).map((fmt) => (
                                                    <button
                                                        key={fmt}
                                                        onClick={() => changeInputFormat(fmt)}
                                                        className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold transition-all duration-200 ${inputFormat === fmt
                                        ...
                                        {decimalOut !== "" && (
                                            <div className="grid grid-cols-2 gap-3">
                                                <StatCard
                                                    ...
                                                    value={decimalOut}
                                                    accent="slate"
                                                    active={inputFormat === "decimal"}
                                                />
                                                <StatCard
                                                    label="Percent"
                                                    value={percentOut}
                                                    accent="emerald"
                                                    active={inputFormat === "percent"}
                                                />
                                                {/* Fraction card with visual fraction */}
                                                <div
                                                    className={`rounded-lg border px-3 py-2 shadow-sm transition-all ${inputFormat === "fraction"
                                                            ? "border-blue-500/60 bg-blue-50/40 ring-1 ring-blue-500/20 dark:border-blue-400/60 dark:bg-blue-900/20"
                                                            : "border-slate-200/80 bg-white/50 dark:border-slate-800/60 dark:bg-slate-900/50"
                                                        }`}
                                                >
                                                ...
                                                <div
                                                    className={`rounded-lg border px-3 py-2 shadow-sm transition-all ${inputFormat === "mixed"
                                                            ? "border-blue-500/60 bg-blue-50/40 ring-1 ring-blue-500/20 dark:border-blue-400/60 dark:bg-blue-900/20"
                                                            : "border-slate-200/80 bg-white/50 dark:border-slate-800/60 dark:bg-slate-900/50"
                                                        }`}
                                                >
                            ...
                            <div className={activeTab === "fractionTools" ? "" : "hidden lg:block"}>
                                <Panel title="🧮 Fraction Tools" onClear={clearFractionTools}>
                                    <div className="flex flex-col items-center">
                                        {/* Tool selector toggle */}
                                        <div className="flex gap-2 mb-5">
                                            <button
                                                onClick={() => setFracTool("simplify")}
                                                className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all ${fracTool === "simplify"
                                            ...
                                            <button
                                                onClick={() => setFracTool("mixed")}
                                                className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all ${fracTool === "mixed"
                                                        ? "border-blue-500 bg-blue-500 text-white shadow-sm"
                                                        : "border-slate-200 bg-white/60 text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-200 dark:hover:bg-slate-800"
                                                    }`}
                                            >
                                        ...
                                        {fracTool === "simplify" ? (
                                            <div className="w-full max-w-xs space-y-4">
                                                ...
                                                {simpResultNum !== null && simpResultDen !== null && (
                                                    <div className="mt-3 rounded-lg border border-slate-200/80 bg-slate-50/50 p-3 dark:border-slate-800/60 dark:bg-slate-900/30 text-center">
                                                        <div className="flex items-center justify-center gap-3">
                                                            <FractionDisplay
                                                                numerator={simpResultNum}
                                                                denominator={simpResultDen}
                                                        ...
                                                        <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                                            Decimal: {(simpResultNum / simpResultDen).toFixed(4)}
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="w-full max-w-sm space-y-4">
                                                ...
                                                <div className="flex justify-center gap-2">
                                                    <button
                                                        onClick={() => setMixedDirection("toImproper")}
                                                        className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold transition-all ${mixedDirection === "toImproper"
                                                                ? "border-blue-500 bg-blue-500 text-white shadow-sm"
                                                                : "border-slate-200 bg-white/60 text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-200 dark:hover:bg-slate-800"
                                                            }`}
                                                    >
                                                    ...
                                                    <button
                                                        onClick={() => setMixedDirection("toMixed")}
                                                        className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold transition-all ${mixedDirection === "toMixed"
                                                                ? "border-blue-500 bg-blue-500 text-white shadow-sm"
                                                                : "border-slate-200 bg-white/60 text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-200 dark:hover:bg-slate-800"
                                                            }`}
                                                    >
                                                ...
                                                {mixedDirection === "toImproper" ? (
                                                    <div className="flex justify-center items-center gap-2">
                                                        <input
                                                            type="number"
                                                            value={mWhole}
                                                            onChange={(e) => setMWhole(e.target.value)}
                                                            placeholder="Whole"
                                                        ...
                                                        <input
                                                            type="number"
                                                            value={mDen}
                                                            onChange={(e) => setMDen(e.target.value)}
                                                            placeholder="Den"
                                                            className="w-16 rounded-lg border border-slate-300 bg-white/80 px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-900/60"
                                                            onKeyDown={(e) => e.key === "Enter" && handleMixedConvert()}
                                                        />
                                                    </div>
                                                ) : (
                                                    <div className="flex justify-center items-center gap-2">
                                                        <input
                                                            type="number"
                                                            value={iNum}
                                                            onChange={(e) => setINum(e.target.value)}
                                                            placeholder="Num"
                                                            className="w-20 rounded-lg border border-slate-300 bg-white/80 px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-900/60"
                                                        />
                                                        <span className="text-sm font-bold">/</span>
                                                ...
                                                {convDisplay && (
                                                    <div className="mt-3 rounded-lg border border-slate-200/80 bg-slate-50/50 p-3 dark:border-slate-800/60 dark:bg-slate-900/30 text-center">
                                                        <div className="flex items-center justify-center gap-2">
                                                            ...
                                                            {convWhole !== undefined && convResultNum > 0 ? (
                                                                <FractionDisplay
                                                                    numerator={convResultNum}
                                                                    denominator={convResultDen}
                                                                    whole={convWhole}
