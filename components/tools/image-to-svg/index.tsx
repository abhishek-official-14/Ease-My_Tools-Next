...
export default function ImageToSvg() {
  ...
  const getModeSettings = (): Record<string, number | boolean> => {
    switch (mode) {
      case "logo":
        return {
          ltres: 0.1, qtres: 0.1, pathomit: 1, colorsampling: 2,
          numberofcolors: quality, mincolorratio: 0, colorquantcycles: 3,
          scale: 1, strokewidth: 1, blurradius: 0, blurdelta: 4, roundcoords: 1,
        };
      case "detailed":
        return {
          ltres: 1, qtres: 1, pathomit: 6, colorsampling: 2,
          numberofcolors: quality + 1, mincolorratio: 0, colorquantcycles: 2,
          scale: 1, strokewidth: 1, blurradius: 1, blurdelta: 10,
        };
      case "sketch":
        return {
          ltres: 4, qtres: 4, pathomit: 12, colorsampling: 0, numberofcolors: 2,
          colorquantcycles: 1, scale: 1, strokewidth: 2, blurradius: 0, blurdelta: 20,
        };
      case "anime":
        return {
          ltres: 0.5, qtres: 0.5, pathomit: 2, colorsampling: 2, numberofcolors: 8,
          colorquantcycles: 2, scale: 1, strokewidth: 1, blurradius: 1, blurdelta: 6,
        };
      case "icon":
        return {
          ltres: 0.1, qtres: 0.1, pathomit: 0, colorsampling: 0, numberofcolors: 4,
          colorquantcycles: 1, scale: 1, strokewidth: 1,
        };
      case "blackwhite":
        return {
          ltres: 1, qtres: 1, pathomit: 1, colorsampling: 0, numberofcolors: 2,
          colorquantcycles: 1, scale: 1, strokewidth: 1,
        };
      case "smooth":
        return {
          ltres: 2, qtres: 2, pathomit: 8, colorsampling: 2, numberofcolors: quality,
          colorquantcycles: 2, scale: 1, strokewidth: 1, blurradius: 2, blurdelta: 12,
          roundcoords: 2,
        };
      case "minimal":
        return {
          ltres: 4, qtres: 4, pathomit: 20, colorsampling: 0, numberofcolors: 3,
          colorquantcycles: 1, scale: 1, strokewidth: 1,
        };
      default:
        return {};
            ...
            <div className="space-y-5">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                            {file?.type.split("/")[1]?.toUpperCase() || "IMG"} · {formatSize(file?.size || 0)}
                          </p>
                        </div>
                        <button
                          onClick={handleReset}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-slate-200/80 bg-white/60 px-3 py-2 text-sm text-slate-800 transition focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400 dark:border-slate-800/60 dark:bg-slate-900/60 dark:text-slate-100"
                >
              ...
              <button
                ...
                className={`w-full rounded-lg px-3 py-2 text-xs font-bold tracking-wide text-white transition-all active:scale-[0.98] ${!preview || loading
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  ...
                  <div className="flex min-h-[200px] items-center justify-center rounded-lg border border-slate-200/80 bg-white/60 p-4 dark:border-slate-800/60 dark:bg-slate-800/30">
                    {preview ? (
                      ...
                      <div className="hidden lg:block text-center opacity-60">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">No image uploaded</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex min-h-[200px] items-center justify-center rounded-lg border border-slate-200/80 bg-white/60 p-4 dark:border-slate-800/60 dark:bg-slate-800/30">
                    {svgUrl ? (
                      <img src={svgUrl} alt="SVG" className="max-h-48 object-contain" />
                    ) : (
                      <div className="hidden lg:block text-center opacity-60">
                        <div className="mb-2 text-5xl">✨</div>
                        <p className="text-xs text-slate-500 dark:text-slate-400">SVG will appear here</p>
                      </div>
                    )}
                  </div>
                </div>
